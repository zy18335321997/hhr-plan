...
先把客户变更拆成新的 `REQ-*`，更新受影响的角色、场景、实体、状态机、规则、
权限、视图、按钮、通知和 traceability。缺少真实项目证据时：

- 明确列出必须读取的表、字段、PID 和节点；
- 仍给出带假设标记的条件化完整方案；
- 不伪造精确 ID，不以一句“缺少上下文”代替业务设计。

状态机变更必须同时说明旧状态兼容、存量数据迁移、并发保护和失败恢复。
...
- □ 新增状态是否进入现有视图筛选和看板分组？
- □ 新按钮的可见条件、可执行角色和目标工作流是否一致？
- □ 退回/拒绝/撤回后，角色是否有可继续处理的视图和按钮？
- □ 字段权限、行范围和通知对象是否覆盖所有交互角色？
- □ 每个新增 `REQ-*` 已追踪到字段、状态转换、工作流及至少一个用户入口？
...
- [x] 工作流修改操作已完成安全检查清单
- [x] 修改前/后节点链路对照已输出
- [x] design_ir、视图、按钮、权限和需求追踪已同步更新
- [ ] **Next**: 自动进入 Step 3
```
