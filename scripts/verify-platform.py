...
def normalize_node_ids(node: dict) -> dict:
    """Return a copy with canonical snake_case IDs.

    Legacy ``typeId``/``actionId`` remain accepted at input boundaries.  If a
    document supplies both spellings they must agree; silently choosing one
    would make the lock non-deterministic.
    """
    normalized = dict(node)
    errors = []
    for canonical, legacy in (("type_id", "typeId"), ("action_id", "actionId")):
        ...
        if (
            ...
            errors.append(
                f"{canonical}={canonical_value} 与旧字段 {legacy}={legacy_value} 冲突"
            )
        normalized[canonical] = (
            canonical_value if canonical_value is not None else legacy_value
        )
    normalized["_id_errors"] = errors
    return normalized
...
def load_nodes_from_lock(lock_path: str) -> dict:
    ...
    for wf in lock.get("workflows", []):
        ...
        for node in wf.get("node_chain", []):
            nodes.append(normalize_node_ids({
...
def load_nodes_from_file(nodes_path: str) -> dict:
    ...
    for workflow in data.get("workflows", []):
        workflow["nodes"] = [
            normalize_node_ids(node) for node in workflow.get("nodes", [])
        ]
    return data
...
def check_type_id_exists(node: dict, node_idx: int, wf_name: str) -> dict:
    node = normalize_node_ids(node)
    if node["_id_errors"]:
        return {
            "result": "fail",
            "detail": f"节点 [{node_idx}] {'; '.join(node['_id_errors'])}",
            "fix": "只保留规范字段 type_id/action_id，或确保旧字段值完全一致",
        }
    tid = node.get("type_id")
    if tid is None:
        return {"result": "fail", "detail": f"节点 [{node_idx}] 缺少 type_id", "fix": "补充 type_id"}
    if tid not in VALID_TYPE_IDS:
        return {"result": "fail", "detail": f"节点 [{node_idx}] type_id={tid} 不在支持的 39 种类型中",
                "fix": f"检查 type_id 是否正确, 有效值见 TYPE_NAMES"}
    if tid in DEPRECATED_TYPE_IDS:
        return {"result": "fail", "detail": f"节点 [{node_idx}] {DEPRECATED_TYPE_IDS[tid]}",
                "fix": "替换为 type_id=26 (发起审批流程)"}
    return {"result": "pass", "detail": f"type_id={tid} ({TYPE_NAMES.get(tid, '')})"}
...
def check_action_id(node: dict, node_idx: int, wf_name: str) -> dict:
    ...
    if tid not in VALID_ACTION_IDS:
        return {"result": "pass", "detail": "无需 action_id 检查"}

    if aid is None:
        type_name = TYPE_NAMES.get(tid, str(tid))
        return {"result": "fail", "detail": f"节点 [{node_idx}] {type_name} 缺少 action_id",
                "fix": "补充明确的 action_id；验证器不会猜测子模式"}

    valid_ids = VALID_ACTION_IDS.get(tid, [])
    if aid not in valid_ids:
        return {"result": "fail", "detail": f"节点 [{node_idx}] type_id={tid} action_id={aid} 不在有效子模式列表中 (有效值: {valid_ids})",
                "fix": f"从有效子模式中选择: {valid_ids}"}
    return {"result": "pass", "detail": f"action_id={aid} ✓"}
...
def check_fetch_mode(node: dict, node_idx: int, wf_name: str) -> dict:
    ...
    if tid == 6 and node.get("action_id") == 20:
...
def check_platform_traps(node: dict, node_idx: int, wf_name: str, all_nodes: list) -> dict:
    ...
    if tid == 17:
        ...
        push_indices = [
            ...
            if normalize_node_ids(n).get("type_id") == 17
        ]
...
def check_topology(workflow: dict, wf_idx: int) -> dict:
    ...
    if total <= 10:
        ...
        # 检查是否有子流程封装
        has_pbc = any(
            normalize_node_ids(n).get("type_id") in (16, 20) for n in nodes
        )
        if has_pbc:
            results["total_nodes"] = {"result": "pass", "count": total,
    ...
    sub_pids = []
    for n in nodes:
        normalized = normalize_node_ids(n)
        if normalized.get("type_id") == 16:
            cfg = n.get("config", {})
...
def main():
    ...
    for wf_idx, wf in enumerate(workflows):
        ...
        nodes = [normalize_node_ids(node) for node in wf.get("nodes", [])]

        for node_idx, node in enumerate(nodes):
            node_result = {
                "node_name": node.get("name", f"节点#{node_idx}"),
                "node_type": f"{TYPE_NAMES.get(node.get('type_id'), 'unknown')} type_id={node.get('type_id')}",
                "checks": {},
            }
