    ...
    for f in (fields or []):
        ...
        out.append({
            ...
            "type": (
                ...
                else (field_types or {}).get(
                    f.get("fieldId") or f.get("controlId", "")
                )
            ),
            "fieldValue": value if value is not None else f.get("fieldValue", ""),
        })
    return out
...
def collect_tasks(
    ...
    def walk(node_list: list[dict], current_pid: str):
        current_aliases = (alias_maps or {}).get(
            current_pid,
            alias_to_id if current_pid == pid else {},
        )
        for node in node_list:
            if not isinstance(node, dict):
                continue
            nt = node.get("nodeType", "")
            alias = node.get("nodeAlias") or node.get("alias", "")
            config = node.get("config", {}) or {}
            node_id = current_aliases.get(alias, "")
            ...
            if nt in NODE_TYPE_ACTION:
                ...
                t.data = {
                    ...
                    "source_node_id": current_aliases.get(source_alias, ""),
            ...
            if nt in ("get_single", "get_multiple") and config.get("filter"):
                filt = config["filter"]
                wire_filters = _build_filter_wire(filt, current_aliases)
                if wire_filters:
                    t = Task()
                    t.alias = alias
                    t.pid = current_pid
                    ...
                    t.data = {
                        ...
                        "source_alias": (config.get("target") or {}).get(
                            "node", ""
                        ),
                        "select_node_id": current_aliases.get(
            ...
            if inner_nodes:
                inner_pid = inner_pids.get(alias, "")
...
def execute_task(session: Any, task: Task) -> dict:
...
def main():
    ...
    parser.add_argument(
        "--inner-mappings",
        help='按 inner PID 分域的 alias 映射 JSON: {"innerPid":{"alias":"nodeId"}}',
    )
    args = parser.parse_args()

    contract = load_contract(args.contract)
    field_types = {
        reference.get("fieldId"): FIELD_TYPE_IDS.get(reference.get("type"))
        for reference in contract.get("field_references", [])
        if isinstance(reference, dict) and reference.get("fieldId")
    }

    def load_json_argument(raw: str) -> dict:
        path = Path(raw)
        try:
            if path.is_file():
    ...
    alias_to_id: dict[str, str] = {}
    if args.mapping:
        alias_to_id.update(load_json_argument(args.mapping))
    batch_output = {}
    if args.batch_output:
        batch_output = load_json_argument(args.batch_output)
        alias_to_id.update(batch_output.get("aliasToNodeId", {}))

    inner_pids: dict[str, str] = {}
    alias_maps: dict[str, dict[str, str]] = {}
    if args.inner_pids:
        inner_pids.update(load_json_argument(args.inner_pids))
    if args.inner_mappings:
        alias_maps.update(load_json_argument(args.inner_mappings))
    if batch_output:
        for c in batch_output.get("created", []):
            ...
            if ip:
                ...
                inner_map = (
                    c.get("aliasToNodeId")
                    or c.get("nodeMap")
                    or batch_output.get("innerAliasToNodeId", {}).get(
                        c.get("alias", ""),
                        {},
                    )
                )
                if inner_map:
                    alias_maps[ip] = inner_map

    pid = (
        args.pid
        or batch_output.get("pid")
        or batch_output.get("processId")
        or contract.get("workflow", {}).get("pid", "")
    )
    if not pid:
        print(
            json.dumps(
                {
                    "verdict": "fail",
                    "code": "RUNTIME_PID_REQUIRED",
                    "detail": (
                        "缺少物理 PID；请传 --pid，或提供含 pid/processId "
                        "的 --batch-output"
                    ),
                },
                ensure_ascii=False,
    ...
    unresolved = [
        ...
        if (
            ...
            or not task.pid
            or (
                task.data.get("source_alias")
                and not (
                    task.data.get("source_node_id")
                    or task.data.get("select_node_id")
                )
            )
            or any(
                field.get("type") is None
                for field in task.data.get("fields", [])
            )
        )
    ]
    if unresolved:
        print(
            json.dumps(
                {
                    "verdict": "fail",
                    "code": "RUNTIME_NODE_MAPPING_MISSING",
