...
def build_context(project_root):
    ...
    param_names = set()
    ws_id_to_name = {}  # worksheetId → worksheetLabel (for resolving API dataSource refs)
    ...
    for mod_dir in sorted(root.iterdir()):
        ...
        for ws_dir in sorted(mod_dir.iterdir()):
            ...
            ws_label = fields_data.get("name") or \
                       (fields_data.get("data", {}) or {}).get("name") or ws_name
            field_list = fields_data.get("fields", []) or \
                         (fields_data.get("data", {}).get("template", {}) or {}).get("controls", [])

            # Collect worksheetId for ID-to-name resolution
            ws_id = fields_data.get("worksheetId") or \
                    (fields_data.get("data", {}) or {}).get("worksheetId")
            if ws_id:
                ws_id_to_name[ws_id] = ws_label
            ...
            for f in field_list:
                ...
                if ft == 29:  # association
                    target = f.get("targetWorksheetName", "") or f.get("dataSource", "")
                    if target:
    ...
    # ── Resolve dataSource IDs to worksheet names ──
    def resolve_name(key):
        """Resolve a key that might be a worksheetId (24+ hex chars) to actual name."""
        if len(key) == 24 and all(c in '0123456789abcdef' for c in key):
            return ws_id_to_name.get(key, key)
        return key

    resolved_relation_graph = {}
    for src, targets in relation_graph.items():
        resolved_relation_graph[src] = {resolve_name(t) for t in targets}
    resolved_reverse_refs = {}
    for src, refs in reverse_refs.items():
        resolved_reverse_refs[resolve_name(src)] = {resolve_name(r) for r in refs}

    # ── 2. Hub Tables (most referenced) ──────────────────────────
    hub_tables = sorted(resolved_reverse_refs.items(), key=lambda x: len(x[1]), reverse=True)
    ...
    ctx = {
        ...
        "relation_graph": {k: list(v) for k, v in resolved_relation_graph.items()},
        "reverse_refs": {k: list(v) for k, v in resolved_reverse_refs.items()},
        "naming": {
