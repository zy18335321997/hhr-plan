...
```bash
# 1. 验证工作表存在 + 字段存在
hap worksheet info WORKSHEET_ID --json

# 2. 对于合约中的每个 fieldId，在返回结果中查找
# 3. 对于 option 类型字段，验证合约中的 option key 在 options[] 中存在
# 4. 验证关联字段的关联目标表正确
```
...
**一次性调用 batch-add，不做分步 node add + save-action**：
...
`--nodes` 的 JSON 数组直接来自执行合约中定义的节点规格列表。每个节点的 DSL 格式见 `references/verified-node-dsl.md`。
